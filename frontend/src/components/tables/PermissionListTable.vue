  ...
  <BaseTable>
    ...
    <tr v-if="isLoading">
      <td colspan="4" class="px-5 py-10 text-center sm:px-6">
        <p class="text-gray-500 text-theme-sm dark:text-gray-400">Loading permissions...</p>
      </td>
    </tr>
    ...
    <tr v-else-if="!filteredPermissions.length">
      <td colspan="4" class="px-5 py-10 text-center sm:px-6">
        <p class="text-gray-500 text-theme-sm dark:text-gray-400">No permissions found.</p>
      </td>
    </tr>
    <tr
    ...
    >
      ...
      <td class="px-5 py-4 sm:px-6">
        <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ permission.name }}</p>
      </td>
      <td class="px-5 py-4 sm:px-6">
        <p class="text-gray-500 text-theme-sm dark:text-gray-400">{{ permission.description || '-' }}</p>
      </td>
      <td class="px-5 py-4 sm:px-6">
        <Badge :color="permission.is_active ? 'success' : 'light'" size="sm">
...
async function fetchPermissions() {
  ...
  try {
    ...
    permissions.value = []
    errorMessage.value = err?.response?.data?.message || 'Failed to load permissions.'
  } finally {
